...
"memoryview(object, remote=False, write=False)\n"
"--\n"
"\n"
...
memoryview(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    static struct {
        ...
        .ob_item = { &_Py_ID(object), &_Py_ID(remote), &_Py_ID(write), },
    };
    #undef NUM_KEYWORDS
    ...
    static const char * const _keywords[] = {"object", "remote", "write", NULL};
    ...
    if (fastargs[1]) {
        remote = PyObject_IsTrue(fastargs[1]);
        if (remote < 0) {
            goto exit;
        }
        if (!--noptargs) {
    ...
    write = PyObject_IsTrue(fastargs[2]);
    if (write < 0) {
        goto exit;
    }
skip_optional_pos:
    return_value = memoryview_impl(type, object, remote, write);
...
/*[clinic end generated code: output=4961c9e97b516188 input=a9049054013a1b77]*/
